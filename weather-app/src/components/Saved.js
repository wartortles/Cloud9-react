...
class Saved extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('http://localhost:8080/weather').then(response => {
      ...
      const result = response.data.weatherData.map((elem, index) => {
        ...
        return (
          <div key={response.data.weatherData.indexOf(elem)}>
            <li className="flex-item" style={style} data-id={response.data.locations[index].id} onClick={this.onClick}>
              <div className="tileInfo">
                <input className='delete' type="image" src={del} value="Delete" onClick={this.delete} data-id={response.data.locations[index].id}/>
                <div className="titleViewAll" data-id={response.data.locations[index].id}>{name}</div>
                <div className="iconViewAll">
                  <img src={icon} className="viewAll_logo"/>
                </div>
                <div className="tempViewAll" data-id={response.data.locations[index].id}>{temp}°</div>

                <div className="bar_tile" style={barStyle}>
                  <p data-id={response.data.locations[index].id}>
                    L {low}° &nbsp;&nbsp; H {high}°</p>
  ...
  onClick(event) {
	...
	delete(event) {
		this.props.delete(event.target.dataset.id);
}
    ...
    return (
      <div>
        ...
        <ul className="flex-container">
          ...
          {this.state.locations.length > 0 && <li className="flex-item" id="extraTile">
            <div className="extraTile">
              ...
              <p>
				...
				{this.state.locations.length === 0 &&
					...
					<img src={wartortle}/>
					<p>Want to know if you'll melt outside? <br/>
				Start getting some weather.</p>
			</div>}
